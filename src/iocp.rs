...
impl CompletionPort {
    ...
    pub fn new(threads: u32) -> io::Result<CompletionPort> {
        // TODO: should use ? to propagate error
        let ret = unsafe { CreateIoCompletionPort(INVALID_HANDLE_VALUE, None, 0, threads) };
        if ret.0 == 0 {
    ...
    pub fn add_handle<T: AsRawHandle + ?Sized>(&self, token: usize, t: &T) -> io::Result<()> {
        self._add(token, HANDLE(t.as_raw_handle() as _))
    }
    ...
    pub fn add_socket<T: AsRawSocket + ?Sized>(&self, token: usize, t: &T) -> io::Result<()> {
        self._add(token, HANDLE(t.as_raw_socket() as _))
    }
...
impl FromRawHandle for CompletionPort {
    unsafe fn from_raw_handle(handle: *mut std::ffi::c_void) -> CompletionPort {
        CompletionPort {
            handle: Handle::new(HANDLE(handle as _)),
        }
    }
}
